...
trait RentAndPassTrait
{
    ...
    private function generateCustomCodeAndSendIt($token, $keypass, $rented_slot_id, $start_date_time, $end_date_time)
    {
        ...
        curl_setopt_array(
            $curl,
            array(
                CURLOPT_URL => 'https://api.rentandpass.com/api/password?clientId=' . $clientId . '&token=' . $token . '&ID=' . $lockid . '&password=' . $keypass . '&name=Hotefit-' . $rented_slot_id . '&startDate=' . $start_date_time . '000&endDate=' . $end_date_time . '000&type=2&reference=hotelfit',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
