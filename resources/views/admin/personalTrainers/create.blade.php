...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.personalTrainer.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.personal-trainers.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('spots') ? 'is-invalid' : '' }}" name="spots[]" id="spots" multiple>
                    @foreach($spots as $id => $spot)
                        <option value="{{ $id }}" {{ in_array($id, old('spots', [])) ? 'selected' : '' }}>{{ $spot }}</option>
                    @endforeach
                </select>
                @if($errors->has('spots'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('expiration') ? 'is-invalid' : '' }}" type="text" name="expiration" id="expiration" value="{{ old('expiration') }}" required>
