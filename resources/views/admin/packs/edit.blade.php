...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pack.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.packs.update", [$pack->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.pack.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $pack->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $pack->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('spot') ? 'is-invalid' : '' }}" name="spot_id" id="spot_id" required>
                    @foreach($spots as $id => $entry)
                        <option value="{{ $id }}" {{ (old('spot_id') ? old('spot_id') : $pack->spot->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('spot'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $pack->price) }}" step="0.01" required>
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $pack->quantity) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $pack->end_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('promo_title') ? 'is-invalid' : '' }}" type="text" name="promo_title" id="promo_title" value="{{ old('promo_title', $pack->promo_title) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('promo_description') ? 'is-invalid' : '' }}" type="text" name="promo_description" id="promo_description" value="{{ old('promo_description', $pack->promo_description) }}">
