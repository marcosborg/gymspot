    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.client-datas.update", [$clientData->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ClientData::PRIMARY_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('primary_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="primary_type_{{ $key }}" name="primary_type" value="{{ $key }}" {{ old('primary_type', $clientData->primary_type) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ClientData::TRAINING_TIME_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('training_time') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="training_time_{{ $key }}" name="training_time" value="{{ $key }}" {{ old('training_time', $clientData->training_time) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('training_frequency') ? 'is-invalid' : '' }}" type="number" name="training_frequency" id="training_frequency" value="{{ old('training_frequency', $clientData->training_frequency) }}" step="1">
